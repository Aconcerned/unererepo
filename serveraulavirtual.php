...
<?php
    ...
    if(isset($_POST['aulavirtualclase']) && $_POST['aulavirtualclase'] >= 0 &&  !$error) {
      $colcdt = $_POST['aulavirtualclase'];
      $error = false;
    } else {
      $error = true;
    }
	...
	if(!$error) {
			$sql = "UPDATE reservaaula SET ".$columns[$colcdt]." = '".$colVal."' WHERE id='".$rowId."'";
