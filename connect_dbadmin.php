...
if (isset($_POST['submit'])) {
  ...
  if (count($errors) == 0) {
    ...
    $mail->Username   = "mailboyforone@gmail.com";  // GMAIL username, nota: debes permitirle al servidor de que permita aplicaciones de otras procedencias, si no no te va a dejar
	...
	$mail->SMTPDebug = 0;
        ...
        $mail->Body = "Usted se ha registrado como administrador en el sistema de reservas de salones de computación de la Universidad Nueva Esparta";
		
        //send mail
        if (!$mail->Send())
        {
            echo "El mensaje no se envio <p>";
            echo "Hubo un error mandando el mensaje: " . $mail->ErrorInfo;
            exit;
        }
    echo '<br></br>';
    echo "Se mando un correo, revise si le llego";
	}
}